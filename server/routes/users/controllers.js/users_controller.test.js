...
describe('Users', () => {
    ...
    describe('management', () => {
        ...
        it('signup existing user', (done) => {
            axios.post(util.buildUrl('users/signup'), {
                email: 'master@dude.com',
                password: 'wrongpasswordhere'  
            }).catch(error => {                
                expect(error.response.status).toEqual(422)
                expect(error.response.data).toEqual({ error: { errno: 2001, code: 'ERROR_EMAIL_EXISTS' } })
                done()
            })
        })
        ...
        it('update session user', (done) => {
            axios.put(util.buildUrl('users/session'), {
                name: 'Triple Dude',
        ...
        it('signin updated user with wrong password', (done) => {
            axios.put(util.buildUrl('users/signin'), {
                ...
                expect(error.response.status).toEqual(422)
                expect(error.response.data).toEqual({ errno: 2000, code: 'ERROR_USER_NOT_FOUND' })
                done()
            })
        })
        ...
        it('forgot error', (done) => {
            axios.put(util.buildUrl('users/forgot'), {
                email: 'nothing@nothing.com'  
            }).catch(error => {
                expect(error.response.status).toEqual(422)
                expect(error.response.data.error).toEqual({errno: 2002, code: 'ERROR_EMAIL_DOES_NOT_EXIST'})
                done()
            })
        })
