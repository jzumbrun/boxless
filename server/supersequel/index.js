...
class Superqequel {
    ...
    outbound(response, request, rows, definition, history) {
        ...
        if(!outboundAjv.validate(definition.outboundSchema, rows)){
            response.queries.push({...this.getQueryName(request), error: {errno: 1005, code: 'ERROR_QUERY_OUTBOUND_VALIDATION', details: outboundAjv.errors}})
        }
        else {
    ...
    query(request, definition, config, user = {}, history = {}) {
        const expression = this.hbs.instance.compile(definition.expression)({...request.properties || {}, '$user': user, '$history': history})
    ...
    queryError(error, request, response, config) {
        ...
        let err = {...this.getQueryName(request), error: {errno: 1006, code: 'ERROR_IMPROPER_QUERY_STATEMENT'}}
        ...
        try {
            ...
            for(const query of config.queries) {
                ...
                if(!this.validateRequest(query, inboundAjv)){
                    response.queries.push({...this.getQueryName(query), error: {errno: 1000, code: 'ERROR_REQUEST_VALIDATION', details: inboundAjv.errors}})
                    continue
                }
                ...
                if(!this.validateQueryDefinition(definition, inboundAjv)){
                    response.queries.push({...this.getQueryName(query), error: {errno: 1001, code: 'ERROR_QUERY_DEFINITION_VALIDATION', details: inboundAjv.errors}})
                    continue
                }

                // Do we have sql?
                if(!definition){
                    response.queries.push({...this.getQueryName(query), error: {errno: 1002, code: 'ERROR_QUERY_NOT_FOUND'} })
                    continue
                }

                // Do we have access rights?
                if(!intersection(definition.access, config.user.access).length){
                    response.queries.push({...this.getQueryName(query), error: {errno: 1003, code: 'ERROR_QUERY_NO_ACCESS'} })
                    continue
                }
                
                // Do we have proper inbound query schema?
                if(!inboundAjv.validate(definition.inboundSchema, query.properties))
                    response.queries.push({...this.getQueryName(query), error: {errno: 1004, code: 'ERROR_QUERY_INBOUND_VALIDATION', details: inboundAjv.errors}})
                else {
                    let queryPromise = this.query(query, definition, config, config.user, history)
                    .then((rows) => {
            ...
            if (typeof config.realease === 'function')
